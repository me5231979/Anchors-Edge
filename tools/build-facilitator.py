#!/usr/bin/env python3
"""Generate the facilitator edition + printable guide for every course.

    python3 tools/build-facilitator.py            # all courses
    python3 tools/build-facilitator.py oct-monday # one course

Mirrors the learner site (courses/<slug>/index.html) and renders the
runbook (facilitator/notes.json) as a notes rail beside each slide,
prepends the briefing page, and writes the printable guide.html.
"""
...
def build(slug):
    ...
    learner_url = SITE + 'courses/' + slug + '/'
    s = open(os.path.join(cdir, 'index.html')).read()
    notes = json.load(open(os.path.join(cdir, 'facilitator', 'notes.json')))
    ...
    s = s.replace(" | Anchor's Edge | Vanderbilt</title>",
                  ", Facilitator Edition | Anchor's Edge | Vanderbilt</title>", 1)
    s = re.sub(r'<link rel="canonical"[^>]*>\n', '', s)
    s = s.replace('<meta name="viewport"', '<meta name="robots" content="noindex">\n<meta name="viewport"')

    # ---- 3. QR points at the LEARNER site (already does via data-url) ----
    s = s.replace('Welcome. Scan or click the chat link to bring this <em class="gold-text">to your screen</em>.',
                  'Learners scan this code or use the chat link; your screen keeps <em class="gold-text">the notes</em>.')
...
'''    <img class="nav__vicon" src="../../../assets/img/vu-v-icon-nav.png" alt="Vanderbilt University" width="34" height="25">
    ...
    def rail(sec):
        ...
        if sec.get('coreNote'):
            parts.append(f'<p class="facnote__meta facnote__core"><b>30-min core:</b> {esc(sec["coreNote"])}</p>')
    ...
    tough_box = f'<div class="brief__box brief__wide"><h3>06 · Tough questions, ready answers</h3><p class="brief__sub">Say each answer out loud once before you teach</p>{tough}</div>' if tough else ''
    ...
    goal_line = f'<p class="why" style="margin-top:1rem"><b>The goal on the wall:</b> {esc2(meta["goal"])}</p>' if meta.get('goal') else ''
    ...
    <div class="wrap wrap-wide">
      ...
      <p class="lead">Two paths: Full ({meta['paths']['full']['minutes']} min) or Core ({meta['paths']['core']['minutes']} min). Every page ahead carries your script in the right rail: Say, Do, Ask with expected answers, Debrief, and a Transition into the next page, with the Core cut named at the bottom of each card. This page is everything that happens before and around the session. Learners never see this edition; the welcome QR on the next page sends them to their own.</p>
      {goal_line}
...
"""
    ...
    # ---- 7. footer marker ----
    s = s.replace("· Anchor's Edge</span>",
                  "· Anchor's Edge · Facilitator Edition · learner link: <a href=\"" + learner_url
                  + '" style="color:rgba(255,255,255,.7);text-decoration:underline">'
                  + learner_url.replace('https://', '') + '</a></span>', 1)

    out = os.path.join(cdir, 'facilitator', 'index.html')
    open(out, 'w').write(s)

    write_guide(cdir, notes)
    print('wrote courses/%s/facilitator/ (index.html %d rails + guide.html)' % (slug, count))


def write_guide(cdir, notes):
    ...
    core_total = sum(x.get('coreMinutes', 0) or 0 for x in gsecs)
...
<title>Facilitator Guide (printable) · {html.escape(gm['program'])}</title>
...
<div class="sheet">
  <header>
    <div>
      <p class="eyebrow">Vanderbilt · Anchor's Edge · ATD facilitator guide · print edition</p>
      <h1>{html.escape(gm['program'].split(' · ')[0])}, the <em>facilitator guide</em></h1>
    </div>
  ...
  <div class="front">
    ...
    {'<h3>The goal this month serves</h3><p>' + _g(gm['goal']) + '</p>' if gm.get('goal') else ''}
    ...
    {'<h3>Tough questions, ready answers</h3>' + tough_html if tough_html else ''}
...
"""
    open(os.path.join(cdir, 'facilitator', 'guide.html'), 'w').write(guide)
...
@media (min-width: 1200px) {
  ...
  body.fac .slide > .hero__canvas, body.fac .slide > .hero__scrim { grid-column: 1 / -1; }
  body.fac .facnote { grid-column: 2; align-self: start; position: sticky; top: calc(var(--nav-h) + 16px);
    margin: 0 18px 0 0; }
  body.fac .dots { display: none; }
}
.facnote {
